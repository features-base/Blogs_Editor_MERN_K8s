...
function GoogleSignIn () {
    ...
    useEffect(() => {
        var REACT_HOST_URL = InMemoryStore.store.cache.env.REACT_HOST_URL
    
        if(!REACT_HOST_URL) 
            REACT_HOST_URL = process.env.REACT_HOST_URL
        
        if(!REACT_HOST_URL) REACT_HOST_URL = "https://accounts.google.com/o/oauth2/v2/auth?redirect_uri=https%3A%2F%2Flocalhost%3A3000&client_id=639506257680-1fa5smdb926pfacdgmhv8ijb882ujn9c.apps.googleusercontent.com&response_type=id_token%20token&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fuserinfo.email+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fuserinfo.profile+openid&flowName=GeneralOAuthFlow&nonce=n-0S6_WzA2Mj&key%3D=AIzaSyC0VfTk_XGGIoUy9Kj18xdR6Re3gMY_dLs"
        setOAuth2Endpoint(REACT_HOST_URL+'api/user/login')
        InMemoryStore.subscribe(() => {
            console.log(InMemoryStore.store.cache.env.REACT_HOST_URL)
            if(InMemoryStore.store.cache.env.REACT_HOST_URL)
                setOAuth2Endpoint(InMemoryStore.store.cache.env.REACT_HOST_URL+'api/user/login')
        })
